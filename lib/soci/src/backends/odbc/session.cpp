...
void odbc_session_backend::begin(const char* beginTx)
{
    ...
    if (is_odbc_error(rc))
    {
        throw odbc_soci_error(SQL_HANDLE_DBC, hdbc_, "Cannot begin transaction.");
    }
}
...
void odbc_session_backend::commit(const char* commitTx)
{
    ...
    if (is_odbc_error(rc))
    {
        throw odbc_soci_error(SQL_HANDLE_DBC, hdbc_, "Cannot commit transaction.");
    }
    reset_transaction();
}
...
void odbc_session_backend::rollback(const char* rollbackTx)
{
    ...
    if (is_odbc_error(rc))
    {
        throw odbc_soci_error(SQL_HANDLE_DBC, hdbc_, "Cannot rollback transaction.");
    }
    reset_transaction();
}
